<template>
  <div class="dashboard-container">
                ...
                <div class="treeCon clearfix">
                    ...
                    <div class="fr">
                      <span class="treeRinfo">
                        <div class="treeRinfo">
                          <span>公司法人代表:{{departData.companyMessage}}</span>
    ...
    <!-- 第四步引入组件 ,这个组件是我们抽取到components的下的组件,v-bind:is 指定是模型的名称（data(){}中的 ）ref是取别名-->
...
export default {
  //第二步在这里可以给我们需要引用的组件取一个名称,绑定到模型中，这里我们就叫deptAdd
  components : {deptAdd},

  data() {
    return {
      //第三步我们可以对引入的组件取一个名称，这里我们就叫deptAdd
      deptAdd : 'deptAdd',
      activeName : 'first', 
      departData : {},
      depts : []
  ...
  methods: {
    //添加部门
    handlAdd(parentId){
      //this.$refs.addDept 是指调用子属性中的dept数据,其中addDept是组件的别名
      //每一次打开添加窗口都应该清除数据，防止之前查看的数据显示
